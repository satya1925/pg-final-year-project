...
def open_enrollment():
    ...
    def save_student():
        ...
        with get_conn() as conn:
            ...
            cur.execute(
                """
                INSERT INTO students (reg_no, name, course, mobile, photo_path, qr_path)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
...
def open_attendance():
        ...
        if stored_encoding[0] is not None and running_match[0]:
            ...
            if encs:
                ...
                if consecutive[0] >= REQ_CONSEC:
                    ...
                    status_lbl_s.config(
                        text=f"Attendance saved: {name_var.get()} ({pct:.2f}%)"
                    )
    ...
    course_var = tk.StringVar()
    course_combo = ttk.Combobox(top_bar_c, textvariable=course_var, width=28, state="readonly")
...
grid = tk.Frame(center, bg="#121212")
grid.pack()
make_card(grid, "📝", "Enroll Student", open_enrollment).grid(row=0, column=0, padx=24, pady=18)
make_card(grid, "👥", "Attendance", open_attendance).grid(row=0, column=1, padx=24, pady=18)
...
# bottom bar
bottom = tk.Frame(root, bg="#121212", pady=10)
